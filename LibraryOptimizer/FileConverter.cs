...
public class FileConverter
{
    ...
    public FileConverter(string inputFilePath, double bitRate, bool isNvidia) : this(inputFilePath)
    {
        ...
        else
        {
            ...
            if (bitRate >= 12)
            {
                _encodeAv1Command = $"ffmpeg -i '{inputFilePath}' -map 0:v:0 -map 0:a? -map 0:s? -c:v av1_qsv -global_quality 20 -preset 1 -c:a copy -c:s copy -map_metadata 0 -map_chapters 0 '{_commandOutputFile}'";
            }
            else if (bitRate <= 12 && bitRate >= 7)
            {
